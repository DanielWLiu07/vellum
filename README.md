...
```
   ┌─────────────────────────┐                    ┌──────────────────────────┐
   │   YOUR APP (the host)    │                    │   VITALS (the viewer)    │
   │                          │                    │                          │
...
npm run sample:pdf # regenerate the checked-in public/sample.pdf demo document
```

`public/sample.pdf` is generated by `scripts/make-sample-pdf.mjs` and committed.
It is deliberately *not* rebuilt during `npm run build` — that would leave a dirty
working tree on every build. Run `npm run sample:pdf` yourself after editing the
